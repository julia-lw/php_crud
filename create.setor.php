...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($input_setor)) {
        $setor_erro = "Por favor, insira um nome para o setor.";
    } else {
        $setor = $input_setor;
    }
...
if (empty($setor_erro)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "s", $param_setor);
            $param_setor = $setor;
    ...
    <div class="max-w-md mx-auto my-10 p-6 bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="space-y-4">
            <div>
                ...
                <input type="text" name="setor" value="<?php echo htmlspecialchars($setor); ?>" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 <?php echo (!empty($setor_erro)) ? 'border-red-500' : 'border-gray-300'; ?>">
